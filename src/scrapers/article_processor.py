...
def collect_full_articles(articles: List[ArticleDict], query: str, output_dir: Optional[str] = None) -> List[ArticleDict]:
    """
...
def save_articles(articles: List[ArticleDict], query: str, format_type: str, output_dir: Optional[str] = None) -> None:
    ...
    if format_type == 'json':
        ...
        if articles:
            # Define standard fieldnames for CSV
            standard_fieldnames = [
                'title', 'gnews_link', 'decoded_url', 'publisher', 'published_time',
                'full_text', 'authors', 'publish_date', 'summary', 
                'keywords', 'top_image'
            ]
...
def collect_news(query: str, output_format: Optional[str] = None, output_dir: Optional[str] = None) -> List[ArticleDict]:
    """
    ...
    if html_content:
        ...
        if articles:
            ...
            if Config.SCRAPE_FULL_ARTICLES:
                articles = collect_full_articles(articles, validated_query, validated_output_dir)
