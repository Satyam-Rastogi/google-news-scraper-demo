"""
Async module for scraping full article content using readability and newspaper libraries
"""
import asyncio
import logging
import re
...
class AsyncArticleScraper:
    ...
    async def scrape_article(self, article_url: str, article_title: str = "") -> Optional[FullArticleDict]:
        """
        Scrape full article content using newspaper library asynchronously
        ...
        try:
            ...
            article_data = await loop.run_in_executor(None, self.circuit_breaker.call, self._scrape_with_newspaper, article_url)
            return article_data
    ...
    async def scrape_article_with_readability(self, article_url: str, article_title: str = "") -> Optional[FullArticleDict]:
        """
        Scrape full article content using readability library asynchronously
        ...
        try:
            # Run the synchronous scraping in a thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            article_data = await loop.run_in_executor(None, self.circuit_breaker.call, self._scrape_with_readability, article_url)
            return article_data
        except Exception as e:
    ...
    async def scrape_article_simplified(self, article_url: str, article_title: str = "") -> Optional[FullArticleDict]:
        """
        Scrape full article content using simplified parsing asynchronously
        ...
        try:
            # Run the synchronous scraping in a thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            article_data = await loop.run_in_executor(None, self.circuit_breaker.call, self._scrape_with_simplified_parsing, article_url)
            return article_data
        except Exception as e:
