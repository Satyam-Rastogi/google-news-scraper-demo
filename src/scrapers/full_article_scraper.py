...
class FullArticleScraper:
    ...
    def scrape_full_article(self, article_url: str, article_title: str = "", image_mode: str = Config.IMAGE_SCRAPE_MODE) -> Optional[FullArticleDict]:
        """
                ...
                for i, image_url in enumerate(list(article_data.get('images', []))[:5]):
                    try:
                        ...
                        if image_path:
                            ...
                            # If mode is 'download', replace the URL with the local path
                            if image_mode == 'download':
                                article_data['images'][i] = image_path
                    except ValueError as e:
